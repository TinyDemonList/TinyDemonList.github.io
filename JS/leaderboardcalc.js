...
async function fetchMainList() {
  ...
  console.log("Fetched main list data:", dataFour); // Debugging line

  if (!dataFour) {
    console.error("Error: main list data is missing");
    return;
  }

  for (const levelKey in dataFour) {
    const level = dataFour[levelKey];
    levelCreatorPoints[level.name] = level.creatorpoints;
  }

  Object.values(dataFour).slice(0, 51).forEach((level, index) => {
    const levelPosObj = levelPos.find(l => l.name === level.name);
    if (levelPosObj) {
      levelPosObj.req = level.minimumPercent;
    }
  });
...
function appendCreatorData(dataTwo) {
  const allCreatorArray = [];
  const leaderboard = document.getElementById("leaderboard");
  const div = document.createElement("div");
  div.id = "creatorLeaderboard";
  div.style.marginTop = "20px";
  let order = 0;

  for (const key in dataTwo) {
    const person = dataTwo[key];
    const totalCreatorPoints = calculateCreatorPoints(person["Levels Made"]);

    allCreatorArray.push({ name: key, creatorPoints: totalCreatorPoints, readorder: order });
    order++;
  }

  allCreatorArray.sort((a, b) => b.creatorPoints - a.creatorPoints);
  displayCreatorLeaderboard(allCreatorArray, div);
  leaderboard.appendChild(div);
}

function calculateCreatorPoints(levelsMade) {
  return levelsMade.reduce((total, level) => total + (levelCreatorPoints[level] || 0), 0);
}
...
function displayCreatorLeaderboard(allCreatorArray, div) {
  const zeroindex = allCreatorArray.findIndex(person => person.creatorPoints === 0);
  const maxIndex = zeroindex === -1 ? allCreatorArray.length : zeroindex;
  let tiecount = 0;
  let curRank = 0;

  const header = document.createElement("h2");
  header.innerText = "Creator Leaderboard";
  div.appendChild(header);

  for (let i = 0; i < maxIndex; i++) {
    const person = allCreatorArray[i];
    const text = document.createElement("p");

    if (i === 0 || person.creatorPoints !== allCreatorArray[i - 1].creatorPoints) {
      curRank += tiecount + 1;
      tiecount = 0;
    } else {
      tiecount++;
    ...
    const cursc = `display(${person.readorder})`;
    text.innerHTML = `<p class="trigger_popup_fricc" onclick="${cursc}"><b>${curRank}:</b> ${person.name} (${Math.round(person.creatorPoints * 1000) / 1000} creator points)</p>`;
    div.appendChild(text);
  }
}
